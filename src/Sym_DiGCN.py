...
def main(args):
    ...
    data.edge_index, edge_in, in_weight, edge_out, out_weight = F_in_out(data.edge_index, data.y.size(-1))
    ...
    for split in range(splits):
        model = SymModel(data.x.size(-1), num_classes, filter_num=args.num_filter, dropout=args.dropout).to(device)    
        ...
        model.eval()
        preds = model(data.x, data.edge_index, edge_in, in_weight, edge_out, out_weight)
