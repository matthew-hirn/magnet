...
def main(args): 
    ...
    for i in range(10):
        ...
        model = APPNP_Link(x.size(-1), 2, filter_num=args.num_filter, alpha = args.alpha, dropout=args.dropout, K=args.K).to(device)
        ...
        for epoch in range(args.epochs):
            ...
            if save_perform <= best_test_err:
                ...
                early_stopping += 1
            '''
            if save_perform >= best_test_acc:
                best_test_acc = save_perform
                torch.save(model.state_dict(), log_path + '/model'+str(i)+'.t7')
