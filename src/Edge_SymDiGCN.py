...
def main(args): 
    ...
    for i in range(10):
        ...
        edge_index, edge_in, in_weight, edge_out, out_weight = F_in_out(edges, size)
        edge_index, edge_in, in_weight, edge_out, out_weight = edge_index.to(device), edge_in.to(device), in_weight.to(device), edge_out.to(device), out_weight.to(device)
        #################################
        ...
        for epoch in range(args.epochs):
            ...
            model.train()
            out = model(x, edge_index, edge_in, in_weight, edge_out, out_weight, train_index)
            ...
            train_loss, train_acc = 0.0, 0.0
            model.eval()
            out = model(x, edge_index, edge_in, in_weight, edge_out, out_weight, val_index)
        ...
        if args.task != 2:
            ...
            model.eval()
            out = model(x, edge_index, edge_in, in_weight, edge_out, out_weight, val_index)[:,:2]
            pred_label = out.max(dim = 1)[1]
            val_acc = acc(pred_label, y_val)
    
            out = model(x, edge_index, edge_in, in_weight, edge_out, out_weight, test_index)[:,:2]
            pred_label = out.max(dim = 1)[1]
